...
object Day17 extends App {
  def partOne() = {
    val input =  Source.fromFile("C:\\Users\\Asia\\IdeaProjects\\aoc24\\src\\resources\\day-17.txt").getLines().toList
  ...
  def partTwo() = {
    ...
    val potentialJumps = prog.zipWithIndex.filter(_._1 == 3).map(pair => pair._1.toString +"," + pair._2.toString).mkString(" ")
    val potentialOutputs = prog.zipWithIndex.filter(_._1 == 5).map(pair => pair._1.toString +"," + pair._2.toString).mkString(" ")
    println(s"potential Jumps $potentialJumps")
    println(s"potential outputs $potentialOutputs")
    // first jump must be reached before first output
  }
